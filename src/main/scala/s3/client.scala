...
class Bucket(val name: String, val client: AmazonS3) {
  ...
  private def getKeysWithPrefix(prefix: String,
    ...
    val initialSleep = ReadSleep
    val maxRetries = 10
    val backOffCoefficient = 2
    val maxSleep = initialSleep.toMillis.toInt * StrictMath.pow(2, maxRetries).toInt.millis
    val listingsF =
...
object S3Client {
  ...
  def withCredentials(credentials: S3Credentials, configuration: Option[ClientConfiguration] = None): S3Client = {
    ...
    val s3Client: AmazonS3 = configuration.fold(new AmazonS3Client(aWSCredentials)) {
